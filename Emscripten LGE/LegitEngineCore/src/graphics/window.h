...
namespace legit_engine {
   ...
   #define MAX_BUTTONS 32
      ...
      private:
         bool Initialize();
         friend void GLFWkey_callback(GLFWwindow* window, int key, int scancode, int action, int mods);
         friend void GLFWmouse_callback(GLFWwindow* window, int button, int action, int mods);
         friend void GLFWcursor_position_callback(GLFWwindow* window, double xpos, double ypos);
         friend void GLFWscroll_callback(GLFWwindow* window, double xoffset, double yoffset);
      };
      ...
      bool Window::Initialize()
      {
         ...
         {
            std::cout << "[Legit Engine Web]: Successfully created new browser instance." << std::endl;
         }
         return true;
      }
